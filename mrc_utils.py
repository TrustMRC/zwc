...
def attention_predict_fn(inputs,
    ...
    probas_start_tmp = probas_start * inputs[1]

    preds_start = paddle.argmax(probas_start_tmp, axis=1)  # get predictions.

    probas_end = paddle.nn.functional.softmax(logits[1], axis=1)  # get probabilities.

    probas_end_tmp = probas_end * inputs[1]

    preds_end = paddle.argmax(probas_end_tmp, axis=1)  # get predictions.
